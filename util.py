...
def compute_F1(predictions, labels, mode_F1, k_val):
    ...
    # print("labels:",labels)
    idx = predictions.topk(dim=1, k=k_val)[1] #top3所在的位置
    # print("idx:",idx)
    predictions.fill_(0) #把predictions里的值全变成0
    # print("predictions.fill_(0):",predictions)
    predictions.scatter_(dim=1, index=idx, src=torch.ones(predictions.size(0), k_val).cuda()) #把idx的位置都变成1
    # print("predictions.scatter_:",predictions)
    # loss_F1 = model_COCO.ranking_lossT(predictions, labels.float())
    # print("loss_F1:",loss_F1)
    if mode_F1 == 'overall':
        # print('evaluation overall!! cannot decompose into classes F1 score')
        mask = predictions == 1
        # print("mask:",mask)
        TP = (labels[mask] == 1).sum().float() #将类别为正类的预测为正类的个数
        # print("labels[mask] == 1:",labels[mask] == 1)
        # print("TP:",TP)
        tpfp = mask.sum().float() #预测为正类的个数
        # print("tpfp:",tpfp)
        tpfn = (labels == 1).sum().float() #类别为正类的个数
        # print("tpfn:",tpfn)
        p = TP / tpfp
        # print("p:",p)
        r = TP / tpfn
...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        ...
        self.src = opt.src
        att_path = os.path.join('/home/yfl213/newdisk/DEML-main/DEML-main/datasets/NUS_WIDE/wiki_contexts/NUS_WIDE_pretrained_w2v_glove-wiki-gigaword-300')
        file_tag1k = os.path.join('/home/yfl213/newdisk/DEML-main/DEML-main/datasets/NUS_WIDE/NUS_WID_Tags/TagList1k.txt')
        file_tag81 = os.path.join('/home/yfl213/newdisk/DEML-main/DEML-main/datasets/NUS_WIDE/ConceptsList/Concepts81.txt')
        ...
        train_loc = os.path.join('/home/yfl213/newdisk/DEML-main/DEML-main/datasets/NUS_WIDE/features/nus_wide_train.h5')
        self.train_features = h5py.File(train_loc, 'r')
        img_names = load_dict(os.path.join('/home/yfl213/newdisk/DEML-main/DEML-main/datasets/NUS_WIDE/img_names.pkl'))
