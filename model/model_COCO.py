...
class RBF(nn.Module): #基于区域的attention feature(region-based feature)
    ...
    """
    def __init__(self, heads=8, d_model=512, d_ff=512, dropout = 0.1):
...
class SBF(nn.Module): #基于语义的attention feature(semantic-based feature)
    """
...
class DEML(nn.Module):
    ...
    def predict_RBF(self, e_r, vecs, W):
        ...
        # print("classifiers.t::", classifiers.t())
        # print("classifiers.t:",classifiers.t())
        m = tensordot(e_r, classifiers.t())
        # print("m_RBF",m)
        # logits_RBF = torch.topk(m, k=1, dim=1)[0]
        logits_RBF = torch.topk(m,k=6,dim=1)[0].mean(dim=1) #取前6个区域 值比较高的 [32,925]每个样本里都有前6个最高的值取平均作为该样本每个标签的预测值
    ...
    def predict_SBF(self, e_g, vecs, W):
        ...
        # print("m_SBF",m.shape)
        logits_SBF = torch.topk(m,k=6,dim=1)[0].mean(dim=1) #取前6个标签 值比较高的 [32,925]每个样本里都有前6个最高的值取平均作为该样本每个标签的预测值
        return logits_SBF
...
def ranking_lossT(logitsT, labelsT):
    # print("logitsT:",logitsT.shape) #(8, 48)
    # print("labelsT:", labelsT.shape) #(8, 80)
    eps = 1e-8
    ...
    # print("sub_logitsT:",sub_logitsT.shape)
    # print("positive_tagsT:", positive_tagsT.shape)
...
def js_div(p_output, q_output):
    ...
    # print("(p_output + q_output) / 2:",(p_output + q_output) / 2)
    # log_mean_output = (((p_output + q_output) / 2) + eps).log() #KL散度的输入经过log
    # print("log_mean_output:",log_mean_output)
    # q_output = q_output.log()
    # print("log_mean_output:",log_mean_output.size()[0])
    # print("p_output:",p_output.size()[0])
    # print("KLDivLoss(log_mean_output, p_output):",KLDivLoss(log_mean_output, p_output)) #nan问题出在KLDivLoss上
